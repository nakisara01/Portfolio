...
const ActivityItem = ({ activity, onOpenModal }) => (
  <div className="flex flex-col md:flex-row gap-4 my-10">
    ...
    <div className="border-2 border-white p-6 rounded-lg shadow-md md:w-2/3 w-full md:h-72 h-auto flex flex-col justify-between bg-transparent text-white">
      ...
      <div className="mb-2">
        ...
        <p className="text-sm">{activity.technologies}</p>
      </div>
...
const Activity = () => {
  ...
  return (
    <section className="my-20 px-5" id="activities">
      <header className="text-3xl font-bold mb-8">Activities</header>

      {/* 프로젝트 리스트 */}
      {activitiesData.map((activity) => (
        <ActivityItem key={activity.id} activity={activity} onOpenModal={setSelectedActivity} />
      ))}
